    ...
    if (credenciaisSalvas) {
        ...
        infoMessage.innerHTML = `
            <p style="color: #1e40af; font-size: 0.9rem; margin: 0;">
                ℹ️ Dados preenchidos automaticamente
            </p>
        `;
...
// Função para verificar se já existe usuário logado
function verificarUsuarioLogado() {
    const usuarioLogadoLocal = localStorage.getItem('usuarioLogado');
    const usuarioLogadoSession = sessionStorage.getItem('usuarioLogado');
    
    if (usuarioLogadoLocal || usuarioLogadoSession) {
        const usuario = JSON.parse(usuarioLogadoLocal || usuarioLogadoSession);
        
        // Redirecionar automaticamente para a home
        alert(`Você já está logado como ${usuario.nome}. Redirecionando...`);
